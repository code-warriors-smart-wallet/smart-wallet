...
//          filteredSpaces = spaces.filter(space => allowedTypes.includes(space.type));
//       }
...
//          // Set active dropdown if current spaceId matches
//          if (space.id === spaceId) {
//             title = space.name;
...
import { RootState } from "@/redux/store/store";
import { UserPortalView } from "./SideBar";
import { toLocalSpaceType } from "../../utils/utils";
...
function NavBar({
  ...
  view: UserPortalView;
  spaceId: string;
  setSpaceFormToggle: Dispatch<React.SetStateAction<boolean>>;
}) {
  const [isUserMenuOpen, setUserMenuOpen] = useState<boolean>(false);
  const { email, username, spaces } = useSelector(
  ...
  const onInputChange = (text: string) => {
    ...
    navigate(
      `/user-portal/${toLocalSpaceType(selectedSpace.type)}/${selectedSpace.id}/${view}`
    );
  };
      ...
      <div className="px-3 py-3 lg:px-5 lg:pl-3 h-full">
        <div className="flex items-center justify-between h-full">
          <div className="flex items-center justify-start rtl:justify-end w-65">
            <button
              type="button"
              className="inline-flex items-center p-2 text-sm rounded-lg sm:hidden hover:bg-hover-light-primary focus:outline-none focus:ring-2 focus:ring-gray-200 active:ring-hover-light-primary dark:text-gray-400 dark:hover:bg-hover-dark-primary dark:focus:ring-gray-600 dark:active:ring-hover-dark-primary cursor-pointer"
              onClick={() => setSideBarOpen(!isSideBarOpen)}
            >
              <MenuIcon />
            </button>
          ...
          <div className="flex items-center relative">
            <div className="flex items-center ms-3">
              ...
              {isUserMenuOpen && (
                <div
                  className="absolute top-1/2 right-0 z-50 my-4 text-base list-none bg-bg-light-primary divide-y divide-gray-100 rounded-sm shadow-sm dark:bg-bg-dark-primary dark:divide-gray-600"
                  id="dropdown-user"
                >
                  <div className="px-4 py-3" role="none">
                    <p
                    ...
                    <p
                      className="text-sm font-medium text-text-light-primary dark:text-text-dark-primary truncate"
                      role="none"
                    >
                      {email}
                    </p>
