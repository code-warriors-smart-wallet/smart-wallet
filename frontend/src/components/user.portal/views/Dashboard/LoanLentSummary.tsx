...
import { InstallmentState } from "@/interfaces/redux";
import { Installment, InstallmentStatus } from "../LoanRepaymentPlan";

function LoanLentSummary({ currency, loanSummary }: { currency: string, loanSummary: any }) {
    ...
    const interest = loanSummary?.interestReceived?.length > 0 ? Number(loanSummary.interestReceived[0].amount).toFixed(2) : 0
    const principalReceived = loanSummary?.principalReceived?.length > 0 ? Number(loanSummary.principalReceived[0].amount).toFixed(2) : 0;
    const penaltyPaid = loanSummary?.penaltyPaid?.length > 0 ? Number(loanSummary.penaltyPaid[0].amount).toFixed(2) : 0;
    const principalRate = loanAmount > 0 ? (Number(principalReceived) / loanAmount * 100).toFixed(0) : 0;
    ...
    const totalPaid = (Number(principalReceived) + Number(interest) + Number(penaltyPaid)).toFixed(2);
        ...
        <section className="rounded my-3 p-3 border border-border-light-primary dark:border-border-dark-primary *:text-text-light-primary *:dark:text-text-dark-primary">
            ...
            <div className="flex gap-3 *:flex-1 flex-wrap">
                ...
                <div className="pb-2 border-border-light-primary dark:border-border-dark-primary mt-3">
                    ...
                    <h2 className="text-xl font-semibold text-text-light-secondary dark:text-text-dark-secondary">{principalReceived} {currency}</h2>
                </div>
                <div className="pb-2 border-border-light-primary dark:border-border-dark-primary mt-3">
            ...
            <div className="flex gap-3 *:flex-1 flex-wrap">
                <div className="p-2 border border-border-light-primary dark:border-border-dark-primary mt-3">
                    <h1>Principal Due</h1>
                    <div className="mt-3">
                        <div className="text-right w-full text-xs text-text-light-secondary dark:text-text-dark-secondary">{principalReceived} {currency}/{loanAmount} {currency}</div>
...
export default LoanLentSummary;