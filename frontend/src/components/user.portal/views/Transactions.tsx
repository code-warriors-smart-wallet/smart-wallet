...
function Transactions() {
   ...
   const { checkIfPlanExists } = LoanRepaymentPlanService();

   const [allowedParentCategories, setAllowedParentCategories] = useState<any[]>([])
   const [allowedSubCategories, setAllowedSubCategories] = useState<CategoryInfo[]>([])
   const currentSpace = spaces.find(sp => sp.id === spaceid);

   // useEffect to check for active loan repayment plan
   useEffect(() => {
      if (spaceid && (toStrdSpaceType(spacetype || "") === SpaceType.LOAN_LENT || 
          toStrdSpaceType(spacetype || "") === SpaceType.LOAN_BORROWED)) {
          checkForActivePlan();
      } else {
         setHasActivePlan(false); // Reset if not a loan space
      }
   }, [spaceid, spacetype]);

   // function to check if loan has active repayment plan
   const checkForActivePlan = async () => {
      try {
         const exists = await checkIfPlanExists(spaceid || "");
         setHasActivePlan(exists);
      } catch (error) {
      ...
      <>
         ...
         <div className="flex justify-between items-center">
            <h1 className="text-xl text-text-light-primary dark:text-text-dark-primary flex items-center">
               ...
               {hasActivePlan && (
                  <span className="ml-3 text-sm text-yellow-600 dark:text-yellow-400">
                     (Manual transactions disabled - Repayment plan active)
                  </span>
               )}
            </h1>
            <div className="flex justify-end gap-3 items-center">
               ...
               <Button
                  ...
                  title={hasActivePlan ? "Cannot create manual transactions when a repayment plan is active" : ""}
               />
            </div>
         </div>
