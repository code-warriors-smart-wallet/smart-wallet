...
authRouter.post("/register", async (req: Request, res: Response) => {
  try {
    const { email, username, password, currency }: CreateAccountRequest =
      req.body;
    ...
    if (!plan) {
      throw new Error("Starter plan not found in the database");
    }
...
authRouter.post("/resend-otp", async (req: Request, res: Response) => {
  try {
    ...
    if (!existingOTP) {
      ...
      if (
        existingOTP.lastOtpAttemptAt &&
        existingOTP.lastOtpAttemptAt < thirtyMinutesAgo
      ) {
    ...
    if (bcrypt.compareSync(password, user.password)) {
      if (!user.enabled) {
        res.status(401).json({
          ...
          data: {
            message:
              "Your email not verified. Please verify your account first.",
            object: {
              username: user.username,
              email: user.email,
              status: LoginStatus.VERIFICATION_REQUIRED,
...
authRouter.post("/logout", async (req: Request, res: Response) => {
  ...
  const user = await User.findOne({ refreshToken }, { new: true }).select(
    "-password"
  );
  if (user) {
    const updatedUser = await User.findByIdAndUpdate(
